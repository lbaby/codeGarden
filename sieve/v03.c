...
#define set_addr(addr_, prod_)\
	do{\
		(addr_).byte_id  =  (prod_)   / NUM_PER_BYTE;\
		(addr_).bit_value   =   (prod_)- (addr_).byte_id * NUM_PER_BYTE  ;\
	}while(0)
...
mem_entry_t *make_entry(mem_entry_t *m, mygoal_t  mem_total)
{

		m->entry = (uint8_t *)malloc(mem_total);
		if(!m->entry){
			return NULL;
		}
		memset(m->entry, 0, mem_total);
...
mem_entry_t *sieve(mem_entry_t *m, mygoal_t goal)
{
	...
	for(lfactor = 3; lfactor <= s_goal +1 ;){
		...
		while(rfactor <= rgoal ){
			...
			uint8_t *non_prime_byte  =  entry + cur_addr.byte_id;
...
void print_prime(mem_entry_t *m, mygoal_t goal)
{
	...
	while (i <= goal) {
		uint8_t *prime_byte = m->entry + cur_addr.byte_id;
...
int main(int argc, char *argv[])
{
	...
	size_t entry_size = mygoal /  NUM_PER_BYTE  + ( mygoal%NUM_PER_BYTE != 0);
